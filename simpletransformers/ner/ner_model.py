...
from simpletransformers.ner.transformer_models.bert_model import BertForTokenClassification
...
from simpletransformers.ner.transformer_models.modeling_xlm_roberta import XLMRobertaForTokenClassification
...
class NERModel:
    def __init__(
        ...
        **kwargs,
        ...
        self.config.multi_loss = self.args.multi_loss
        self.config.cls_num_labels = self.args.cls_num_labels
        self.config.loss_weight = self.args.loss_weight
        self.config.max_correction_embeddings = self.args.max_correction_embeddings
        self.config.parsing_embedding = self.args.parsing_embedding
        self.config.parsing_embedding_for_embedding = self.args.parsing_embedding_for_embedding
        ...
        **kwargs,
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if self.args.fp16:
                    with amp.autocast():
                        loss, *output = self._calculate_loss(
                            model,
                            inputs,
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.evaluate_during_training and (
                        ...
                        if test_data is not None:
                            test_results, _, _, _ = self.eval_model(
                                test_data,
                                verbose=verbose
    ...
    def eval_model(
        ...
        **kwargs,
        ...
        result, model_outputs, preds_list, out_label_list = self.evaluate(
            eval_dataset,
            eval_data,
            output_dir,
            verbose=verbose,
        ...
        return result, model_outputs, preds_list, out_label_list
        ...
        **kwargs,
        ...
        for batch in tqdm(
            ...
            if preds is None:
                ...
                out_attention_mask = np.append(out_attention_mask, inputs["attention_mask"].detach().cpu().numpy(), axis=0, )
            ...
            out_label = [[] for _ in range(out_label_ids.shape[0])]
            preds_l = [[] for _ in range(out_label_ids.shape[0])]
            label_map = {i: label for i, label in enumerate(self.args.labels_list)}

            for i in range(out_label_ids.shape[0]): # batchs
                for j in range(out_label_ids.shape[1]):
                    if out_label_ids[i, j] != pad_token_label_id:
                        out_label[i].append(label_map[out_label_ids[i][j]])
                        preds_l[i].append(label_map[preds[i][j]])
            out_label_list.extend(out_label)
            preds_list.extend(preds_l)
            ...
            model_outputs = [
                [word_tokens[i][j] for j in range(len(preds_l[i]))]
                for i in range(len(preds_l))
            ]
            preds = None

        result = calc_metrics(self.args,
                 eval_data,
                 model_outputs,
                 out_label_list,
                 preds_list,
                 ...
                 eval_output_dir,
                 wandb_log,
                 tgt_out_label_list=tgt_out_label_list if len(tgt_out_label_list) > 0 else None,
                 tgt_preds_list=tgt_preds_list if len(tgt_preds_list) > 0 else None,
                 src_out_label_list=src_out_label_list if len(src_out_label_list) > 0 else None,
                 src_preds_list=src_preds_list if len(src_preds_list) > 0 else None,
                 **kwargs)
        new_out_label_list = []
        new_preds_list = []
        ...
        return results, model_outputs, preds_list, out_label_list
    ...
    def predict(self, to_predict, split_on_space=True):
        ...
        Args:
            to_predict: ["My mom like watching movie.", 2], ["My mom likes watch movie.", 3]
        ...
        if split_on_space:
            predict_examples = [
                    InputExample(
                        i,
                        sentence[0].split(),
                        [self.args.labels_list[0] for word in sentence[0].split()],
                        correction_index=sentence[1]
                    )
        ...
        if split_on_space:
            preds = [
                [
                    {word: preds_list[i][j]}
                    for j, word in enumerate(sentence[0].split()[: len(preds_list[i])])
                ]
                for i, sentence in enumerate(to_predict)
            ]
        else:
    ...
    def _convert_stbert_output_matrix_to_tokens(self, out_label_ids, preds, label_map):
        label_list = []
        preds_list = []
        target_preds_list = []
        source_preds_list = []
        ...
        for n in range(out_label_ids.shape[0]):
            source_label = [32] * out_label_ids.shape[1]
            target_label = [32] * out_label_ids.shape[1]
            source_pred = [32] * out_label_ids.shape[1]
            target_pred = [32] * out_label_ids.shape[1]

            for i in range(out_label_ids.shape[1]):
                if out_label_ids[n][i].tolist().count(32) > 4:
                    source_label = out_label_ids[n][i]
                    source_pred = preds[n][i]
            source_preds = []
            source_labels = []
            for sl, sp in zip(source_label, source_pred):
                if sl != -100:
                    source_preds.append(label_map[sp])
                    source_labels.append(label_map[sl])

            for j in range(out_label_ids.shape[2]):
                if out_label_ids[n][:, j].tolist().count(32) > 4:
                    target_label = out_label_ids[n][:, j]
                    target_pred = preds[n][:, j]
            target_preds = []
            target_labels = []
            for tl, tp in zip(target_label, target_pred):
                if tl != -100:
                    target_preds.append(label_map[tp])
                    target_labels.append(label_map[tl])

            label_list.append(target_labels + source_labels)
            preds_list.append(target_preds + source_preds)

            target_preds_list.append(target_preds)
            target_label_list.append(target_labels)

            source_preds_list.append(source_preds)
        ...
        if self.args.use_hf_datasets and data is not None:
            ...
            if not to_predict and isinstance(data, str) and self.args.lazy_loading:
                ...
                if to_predict:
                    ...
                    if isinstance(data, str):
                        ...
                        examples = get_examples_from_df(
                            data,
                            correction_index=self.args.max_correction_embeddings,
                            parsing_embedding=self.args.parsing_embedding,
                        )
                ...
                if os.path.exists(cached_features_file) and (
                    ...
                    features = self.gen_features(examples, args, tokenizer, process_count, mode)
                    # if not no_cache:
                    #     torch.save(features, cached_features_file)


                all_tensors = self.feature_to_tensor(features)
                print('feature to tensor finished')
                dataset = TensorDataset(*all_tensors)
                # dataset = Dataset(*all_tensors)
    ...
    def _get_inputs_dict(self, batch):
        if self.args.use_hf_datasets and isinstance(batch, dict):
            ...
            if self.args.max_correction_embeddings:
                inputs["correction_ids"] = batch[4]
            if self.args.max_correction_embeddings and self.args.parsing_embedding:
                inputs["parsing_ids"] = batch[5]
            if not self.args.max_correction_embeddings and self.args.parsing_embedding:
                inputs["parsing_ids"] = batch[4]
            return inputs
    ...
    def gen_features(self, examples, args, tokenizer, process_count, mode):
    ...
    def feature_to_tensor(self, feature):
        ...
        all_label_ids = torch.tensor([f.label_ids for f in feature], dtype=torch.long)
        all_tensors = [all_input_ids, all_input_mask, all_segment_ids, all_label_ids]

        if self.args.max_correction_embeddings:
            all_cor_ids = torch.tensor([f.correction_index for f in feature], dtype=torch.long)
            all_tensors.append(all_cor_ids)
        if self.args.parsing_embedding:
            all_parsing_ids = torch.tensor([f.parsing_ids for f in feature], dtype=torch.long)
            all_tensors.append(all_parsing_ids)
        return all_tensors
